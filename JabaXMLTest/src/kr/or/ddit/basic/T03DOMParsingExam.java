...
public class T03DOMParsingExam {
	...
	public void parse() throws Exception {
         ...
         String recipeId = "423";   // 래시피가 궁금한 음식 ID
		...
		// XML문서를 생성하기 위한 DocumentBuilder객체 생성하기
			...
			if(code.equals("INFO-000")) { // 정상 상태인 경우...
				...
				for(int i=0; i<rowNodeList.getLength(); i++) {
					...
					String itdntCpcty = element.getElementsByTagName("IRDNT_CPCTY").item(0).getTextContent();
					String irdntTyNm = element.getElementsByTagName("IRDNT_TY_NM").item(0).getTextContent();
					
					System.out.printf("%3s %8s %10s %10s %8s\n" , rowNum, recipeId, irdntTyNm, irdntNm, itdntCpcty);
					System.out.println("------------------------------------------------------");
