...
import kr.or.ddit.board.sirvice.BoardServiceImpl;
import kr.or.ddit.board.sirvice.IBoardService;
...
public class BoardMain {
	...
	private void selectAllBoard() {
		...
		List<BoardVO> boardList = boardService.selectAllBoard(); // 회원정보 가져옴
		...
		if(boardList.size() == 0) {
			...
			for(BoardVO mv : boardList) {
				...
				System.out.println(mv.getBoardNum() + "\t" + mv.getBoardTitle() + "\t" + mv.getBoardContent() + "\t" + mv.getBoardWriter()+ "\t" + mv.getBoardDate());
			}
		}
	...
	private void deleteBoard() {
		...
		int cnt = boardService.removeBoard(bordWriter);
		
		if (cnt > 0) {
			System.out.println(bordWriter + "글 삭제 성공~");
		} else {
			System.out.println(bordWriter + "글 삭제 실패!");
		}
	...
	private void updateBoard() {
		...
		do {
			...
			bordWriter = scan.next();

			exist = boardService.checkBoard(bordWriter);

			if (!exist) {
				System.out.println(bordWriter + "님의 글은 존재하지 않습니다.");
		...
		if (cnt > 0) {
			System.out.println(bordWriter + "님의 글 수정 성공~");
		} else {
			System.out.println(bordWriter + "님의 글 수정 실패!");
		}
	}
	 ...
	 */
	  ...
	  if(boardList.size() == 0) { System.out.println("해당 검색어 존재하지 않습니다."); 
	  
	  }else {
	  
	  for(BoardVO mv2 : boardList) {
	  
	  System.out.println(mv2.getBoardNum() + "\t" + mv2.getBoardTitle() + "\t" +
	  mv2.getBoardContent() + "\t" + mv2.getBoardWriter()+ "\t" + mv2.getBoardDate()); } }
