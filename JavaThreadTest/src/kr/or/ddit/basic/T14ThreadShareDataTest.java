...
class ShareData {
	...
	/*
	 * 임계영역이 발생하면 동기화를 해줘야함
	 	volatile => 선언된 변수를 컴파일러의 최적화 대상에서 제외시킨다.
	 	                           즉, 값이 변경되는 즉시 변수에 적용시킨다. 
	 	                           멀티스레드 환경에서 하나의 변수가 완벽하게 한번에 작동하도록 보장하는 키워드
	 	            (일종의 동기화)
	 */
	
	volatile private boolean isOk = false; // 원주율 계산 완료여부 확인용. volatile-최적화대상에서 제외가 되면서..속도는 느려지지만 정확해짐
